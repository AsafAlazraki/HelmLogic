...
| **v1.33.1** | **2026-07-15** | 🚢 Patch — fit-up picker honours the sanctioned hidden class (CREDIT lines can't reach quotes); nightly battery sections M (Display-Sheet composition inputs, all 809 MPF variants, known-gaps ratchet: 31 kits + 1 prop → v1.34 mapping backlog) + N (override drift alarm, 0 shadows) + extract-less crash fix. 32,579/32,579 on shipped commit. No rules changes. | `claude/app-overview-wKiZ1` |
| **v1.33** | **2026-07-14** | 🚢 **Submitted-backlog burn-down + Usage Reporting (Epic 14)** — telemetry engine (activitySessions + activityEvents, active-vs-idle, every click/nav/action, billh tagged TEST) + Reporting → Usage & Activity tab (filters + PDF export); rules published + verified live (telemetry paths, locked-quote soft-delete, isLocked hardening); all 26 submitted items dispositioned; Bill's 11 asks built (per-item DFO remove, Pre-Rig removal, PDF Factory Options heading + Dealer→org-name, PDF whole-row drag, rego stickers, bigger option text, DFO image upload, Factory Configurator on all brands, per-section rebates, ADMINISTRATION step — flow is 7 steps now, Build-A-Boat cash lock). FFR-33 proof re-ran GREEN on the 7-step flow ($103,731 exact). See `tasks/RELEASE_NOTES_v1.33.0.md`. | `claude/app-overview-wKiZ1` |
| **v1.32** | **2026-07-08** | ✅ Shipped (PR #46) — **Display-Sheet pricing engine (FFR-33)** — quotes compose + price exactly like NSM's Display Sheet on every MPF boat: stale override sweep (fleet-wide, 0 shadows), PD tiers on all 809 variants, PD tier + slot rigging + prop auto-composition, inc-GST convention (`pricingConvention: 'display-sheet-v2'`, legacy quotes untouched). Proof: Mark's SP560 rebuilt in-browser + customer PDF at the sheet's own number. See `tasks/RELEASE_NOTES_v1.32.0.md`. | merged |
| **v1.31** | **2026-07-03** | 🚢 **MPF migration + Testing & Evidence overhaul** — Epic 12 (NSM's 17-workbook Master Price File migrated 1:1: 36,551 writes / 0 errors / parity proven 34,498 of 34,512 with every fail explained) + Epic 13 (460-test money-math unit suite, CI gate + nightly synthetic, visual regression + TLS bridge, fail→fix→retest ledger FFR-1…31 incl. the hunt-wave close-out: full fleet walk green, 809-boat relation web ALL SETS EQUAL 0 unexplained, Step-5 curation engine, three adversarial audits, FFR-16 checklist 11/11 solo). **Release-prepped 2026-07-05 — the pending dev → main PR spans v1.18 → v1.31.** See `tasks/RELEASE_NOTES_v1.31.0.md` + `tasks/RELEASE_PR_DRAFT_v1.31.md` + the "v1.31 new Firestore surface" block below. | `claude/app-overview-wKiZ1` |
