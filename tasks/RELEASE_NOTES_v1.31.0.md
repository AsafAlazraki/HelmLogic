...
**Addendum (Hunt-wave close-out, 2026-07-04/05)** — the post-parity adversarial wave, ledger FFR-17…32 (all fixed + re-proven green, incl. FFR-32: GT consoles never offer or charge a seat — the FCT comes standard; 31 mispaired entries fixed across catalog + override, browser-proven with a pairing-intact control). New: `src/lib/hero-carousel.ts` (+15 unit tests, FFR-30 — full image-fallback chain, type-safe carousel scroll targeting), `scripts/mpf/audit-invariants.py` + `tasks/test-evidence/INVARIANTS_AUDIT.md` (9 money invariants over 809 boats + org collections; one real bug found and fixed, two audit formulas corrected against NSM's own sheet formulas, three NSM-source defect classes returned), `scripts/mpf/audit-structure.py` + `STRUCTURE_AUDIT.md` (74 code paths vs deployed rules, live-probed as Bill Hull; P0 gaps rules-fixed + deployed same day, P1s in repo awaiting publish), `tests/ui-audit.spec.ts` + `UI_AUDIT.md` (UI-1…11, all fixed or ledgered), `scripts/mpf/verify-per-boat-sets.py` + `PER_BOAT_SETS.md` (809-boat relation web: **ALL SETS EQUAL, 0 unexplained** after the curation batch), full-fleet walk `tasks/test-evidence/highfield-walk/` (85 models / 640 variants / 32,816 price asserts, all green), `MODULE_QUOTES.md` (every module quotable, Service & Counter Quotes GST-ceil proven on PDF). Modified: `src/lib/step5-curation.ts` (FFR-31 — R-HP engine-count scaling so twin-rig hulls keep their per-engine-named gear; unit suite now 554), `highfield-quote-flow.tsx` (maxEngines context), `firestore.rules` (P1: cross-user notification create + features auditLog write), `scripts/gen-final-report.mjs` (new report section 12 "The hunt"). FFR-16 closed GREEN: Mark's CL380 checklist 11/11 solo on the post-curation build — the two prior failures were environment contention (concurrent Firestore jobs on the sandbox proxy), proven by elimination.
