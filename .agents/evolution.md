...
- **"emailTemplates permission denied" was NOT a rules problem — don't assume drift.** Bill Hull hit a full-page "Something went wrong" on Create Proposal citing an emailTemplates `list` denial. I assumed (again) the deployed rules had drifted and told the user to re-paste — WRONG. When the user pasted their deployed rules they were identical to the repo, `emailTemplates` present + correctly nested + `allow read: if isSignedIn()`. The real bug was in the app: `SendQuoteDialog` (mounted by proposal-view regardless of open state) called `useEmailTemplates()` unconditionally on mount → the query fired on every Create-Proposal load; the query used `orderBy('isDefault').orderBy('updatedAt')` which forces a composite index; any failure there (missing index / propagation / transient) was promoted by the Firebase error-emitter into a throw that the GLOBAL error boundary (`app/global-error.tsx`) rendered as a full-page crash. Two-line root cause, dressed up as a permissions error because the emitted debug JSON shows the auth + path. **Lesson: a Firestore "permission denied" debug payload does NOT prove the rule is missing — verify the deployed rule before prescribing a re-deploy; and trace whether the failing read is even reachable/necessary on that screen.** Fix: gate the subscription on `open`, and drop the composite-index orderBy (sort client-side). Deeper systemic smell: the error-emitter promoting ANY non-critical read failure to a full-page crash means one denied background query white-screens the app — a candidate for a future "non-blocking read" mode so non-essential reads degrade instead of crashing.
...
## v1.10 in flight — Phase A bug pass + Phase B Fit-Up admin (2026-06-02)

- **Don't ship a release for the sake of releasing.** First version of this work shipped as a "v1.9.5.1" patch, with bug fixes + minimal fit-up groundwork (just 9.1.1 + 9.1.2 schema + admin CRUD). When asked "is there substantial work on that release?" the honest answer was no — the fit-up catalog was admin-only with no salesperson consumer until v1.16+. User correctly called it "release for the sake of releasing." The fix: absorb everything into v1.10 (which already has Service Quoting as its anchor), AND expand the fit-up scope to ALL of Epic 9.1.x (CSV import/export + bulk markup + retier/delete) so the admin surface is actually complete. Removed `RELEASE_WINDOWS['v1.9.5.1']`, deleted `RELEASE_NOTES_v1.9.5.1.md` + `USER_GUIDE_v1.9.5.1.md`, renamed the retarget button (`V1951RetargetButton` → `V110RetargetButton`) to cover all four 9.1.x stories. Lesson: a release tag should match the centre of gravity of the work. If a "patch" is mostly bug fixes plus one orphan admin surface, fold it into the next minor.
- **Bug-pass discipline: verify agent-flagged "bugs" before fixing.** Two agent reports during the bug sweep (now Phase A of v1.10) turned out to be false positives, caught by reading the actual code instead of trusting the summary. (a) "Personalisation lock-guard missing — locked quotes silently reject content-override saves" — the `contentOverrides` subcollection has its OWN independent `allow read, write: if isSignedIn()` rule in firestore.rules and does NOT inherit the parent quote-doc lock. Writes succeed. NOT a bug. (b) "Content-block-import silently skips unknown blockTypes" — the toast at line 222 of `content-block-import.tsx` already reports `${skipped} skipped (unknown blockType)`. NOT a bug. Lesson: a fix-bug agent will confidently produce a fix-patch for a non-bug. Always read the implicated code yourself + verify the actual misbehaviour BEFORE applying agent-suggested fixes. Two seconds of `grep` saves a wrong commit.
- **Bug-fix commits should explicitly document what was NOT fixed.** The b516d4e commit message included a "What's NOT in this commit" section: 2 false positives skipped (with why), save + import bugs deferred for repro, RU200KAM $76.82 deferred to v1.11. Future-agent reading this commit can immediately tell what was triaged vs. ignored, no archaeology required.
- **Pull-forward stories need the full flow-on-effects sweep.** Pulling all four Epic 9.1.x stories forward into v1.10 isn't just "build the code" — it touches release-schedule.ts (no new key needed — they fold into v1.10), Firestore feature docs (retarget + status: shipped via one-shot button), the dependency graph (downstream 9.x stories now reference a shipped dep), the capacity packer (slots freed in later releases — buffer absorbs), the release-state tables (CLAUDE.md + SESSION_HANDOVER.md), and the eventual user guide / release notes that ship WITH the v1.10 PR. When the user said "we also need to update all flow on effects as a result of this request" they were codifying the v1.6.2 lesson — cross-cutting changes need a checklist, not vibes.
- **Fit-Up vs DealerFit — sibling but not the same shape.** DealerFit categories are bare strings on the module doc (`moduleDealerFitCategories: string[]`); fit-up items are full docs in an org-level subcollection (`organisations/{orgId}/fitUpItems/{itemId}`) because they carry cost + tier + notes per item, not just a name. Mirror the dealerFit UI pattern (Card + add/edit/remove rows + filter chips) but do NOT mirror the schema. When the salesperson-side ships in Epic 9.2, expect parallels with `dealer-fit-options.tsx` (category grouping + selection accordion) — but the quote payload structure should be a sibling of `quote.dealerFit[]`, not a reuse of it.
- **CSV import is upsert-by-natural-key, never clear-and-replace** (v1.4 remediation re-applied). 9.1.3's import detects the name column from a priority list (`name`, `item name`, `item`, `title`, `description`, `part description`), normalises tier with first-letter detection, and upserts by lowercased name. `N updated · M created · K skipped (no name)` toast every time. The pattern from the v1.4 Master Price File import (Yamaha MPF, Sam Allen, delivered-deals, stock, trailer pricing) applies again — operator edits must survive partial re-imports.
- **Bulk markup is destructive — flag it as such in the confirm dialog.** v1.10's 9.1.4 bulk markup tool *overwrites* any pre-existing sellPrice on selected items. The confirm dialog says so explicitly. Same applies to the bulk-delete action which has its own destructive copy ("There is no undo. Operator-only — no quote references these items yet"). Bulk destructive actions are exactly where confirm-dialog copy needs to spell out the consequence, not just say "Are you sure?".
- **One-shot retarget buttons can be self-contained components, not modules.** v1.10's `V110RetargetButton` is a single ~100-line component dropped into `manage-organisation-page.tsx`'s new Fit-Up tab. No new admin module, no new permissions, no new menu entries — just a tile that lives at the top of an existing surface for one click. Pattern is leaner than the v1.10-restructure workbench (whole sheet + apply pipeline) for simple multi-doc retargets. Keep one-shots proportional to the work.
- **Sister collections deserve sister components, not one over-loaded one** (Phase C learning). Service Quoting catalogue needs two collections (serviceOperations + serviceParts) with different schemas (operations have `flatRateHours × hourlyRate`, parts have `partNumber + stockLevel`). Tempting to write one `ServiceCatalogManager` that conditionally renders both — but the schemas diverge enough that the conditional logic would dominate. Cleaner: one parent component with a `<Tabs>` (Operations / Parts), each tab a self-contained sub-component (`ServiceOperationsTab`, `ServicePartsTab`) with its own editor + bulk dialog. ~700 lines total but each tab is ~300 lines of clean code rather than 700 lines of conditional spaghetti. Sister components, single file — easy to grep, easy to evolve when 11.2.x consumes them.
- **Derived sell-price vs. override needs to be explicit in the schema.** ServiceOperation's `sellPrice` is OPTIONAL by design — when absent, the UI + downstream consumers compute `flatRateHours × hourlyRate`. When set, it's an override that survives bulk markup. The bulk-markup tool deliberately adjusts `hourlyRate` (the lever) rather than overwriting `sellPrice` overrides, so the derivation stays transparent. This pattern will repeat in Epic 11.2 when service quotes line-item the operations — the quote should capture which leg it used (derivation vs. override) so an audit can answer "why did this line price change after a bulk update?".
- **Don't speculatively build "module" or "parts" stories without specific IDs.** The v1.10 restructure plan said `v1.10: 18 pts · 6 stories · fit-up x2, module x3, parts x1 · [2 NEW]`. I built fit-up (4 stories — pulled 9.1.3 + 9.1.4 forward) + Service Quoting catalogue (2 stories). For "module x3" and "parts x1" I had no specific story IDs to build against — they live in Firestore as planning rows and I can't query Firestore from the sandbox. Building speculatively (picking module/parts targets from imagined needs) would have produced code the user doesn't want. Better move: retarget those buckets to v1.11 and surface them at v1.11 kickoff via the planning workbench, where the user can pick the specific stories from Firestore. Pattern: when the planning DB is the source of truth and I don't have read access, narrow scope to what's in the codebase / docs and defer speculative work to a planning conversation.
- **A demo deadline reframes "is it done?" honestly.** When the user asked "have we finished v1.10?", the honest answer was no — and the better answer was "here's exactly what's done, what's missing, here are three options for the demo." Don't fake completeness. The roadmap reflects truth: shipped stories show as shipped (after retarget button click), unbuilt stories stay planned. A partial v1.10 with a clear "Phase C done, Phase D = v1.11" narrative beats a panicked sprint that produces broken code in front of stakeholders.
- **The FirebaseErrorListener denylist is the v1.9 lesson finally landing.** The v1.9 evolution entry warned: *"a candidate for a future 'non-blocking read' mode so non-essential reads degrade instead of crashing."* I noted it then. I didn't apply it. v1.10 hit the SAME class of bug FOUR times before I finally fixed the listener: every time the user added a new collection rule that wasn't yet deployed, the global error boundary white-screened the app. The lesson re-stated bluntly: **when an evolution lesson identifies a systemic smell with a known fix, write the code, don't just narrate the lesson.** The fix (`FirebaseErrorListener` denylist, commit `e58f4cf`) is six lines of logic — should have been the FIRST response to the Bill Hull crash, not the fifth. Going forward, any NEW collection added in a release auto-graduates to a denylist candidate until rules ship.
- **One-shot lifecycle scales across multiple buttons in one cycle.** v1.10 close-out shipped THREE one-shots in different lifecycle states simultaneously: `V110RetargetButton` + `V110BumpUnbuiltButton` (both v1.10-stories tooling, user clicked, removed in this commit) AND `V195StoriesSeedButton` (just-added v1.9.5 backfill, user clicks, removed in next commit). Each lives on the Roadmap header alongside the others. The pattern works because each button is self-contained + idempotent — the user can click in any order, re-click safely, and each one's life cycle progresses independently. Don't try to consolidate one-shots into a single "do everything" button; per-purpose buttons stay readable + safe to roll back individually.
- **Backfill the planning rows when a release ships without them.** v1.9.5 shipped to prod (PR #36) with the real work — roadmap reshuffle, Service Quoting groundwork seed, clickable popups, Create-Proposal hotfix, Workbench tooling — but no `features` documents were ever created to represent that work on the planning board. The Roadmap rendered v1.9.5 as "0 activities · 0 pts" even though it was a sizeable release. The fix is a one-shot backfill seed (`V195StoriesSeedButton`) that creates the missing story docs as `status: shipped, targetRelease: 'v1.9.5'` so the timeline reflects reality. Generalisation: every release-notes file should have a corresponding set of `features` docs. When the release notes describe work that doesn't have story coverage, seed retroactively. Cheap, makes the planning board honest, helps stakeholder demos.
- **Boats Catalogue (Story 3.7.2) is a read-only fast-audit surface, not a competing editor.** Cleanly separating "edit catalogue" (existing module-page editors) from "audit catalogue" (the new `/boats` read-view) avoids the URL-state-cascade problems from v1.3.1. The audit page has its own URL, its own data fetch, lazy-loads variants per-row, and never touches Firestore writes. Patterns to repeat: any "spot anomalies across the whole dataset" surface should be read-only, lazy-load nested data, and live on its own route — don't bolt it onto an existing edit page where state collisions are inevitable.
