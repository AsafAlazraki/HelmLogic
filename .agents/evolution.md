...
## v1.9.5.1 Patch — bug pass + Fit-Up groundwork (2026-06-02)

- **Bug-pass discipline: verify agent-flagged "bugs" before fixing.** Two agent reports during the v1.10 (rolled into v1.9.5.1) bug sweep turned out to be false positives, caught by reading the actual code instead of trusting the summary. (a) "Personalisation lock-guard missing — locked quotes silently reject content-override saves" — the `contentOverrides` subcollection has its OWN independent `allow read, write: if isSignedIn()` rule in firestore.rules and does NOT inherit the parent quote-doc lock. Writes succeed. NOT a bug. (b) "Content-block-import silently skips unknown blockTypes" — the toast at line 222 of `content-block-import.tsx` already reports `${skipped} skipped (unknown blockType)`. NOT a bug. Lesson: a fix-bug agent will confidently produce a fix-patch for a non-bug. Always read the implicated code yourself + verify the actual misbehaviour BEFORE applying agent-suggested fixes. Two seconds of `grep` saves a wrong commit.
- **Bug-fix commits should explicitly document what was NOT fixed.** The b516d4e commit message included a "What's NOT in this commit" section: 2 false positives skipped (with why), save + import bugs deferred for repro, RU200KAM $76.82 deferred to v1.11. Future-agent reading this commit can immediately tell what was triaged vs. ignored, no archaeology required. Same pattern in the release notes — `tasks/RELEASE_NOTES_v1.9.5.1.md` has explicit "Process notes" listing the four deferrals + the two false positives.
- **Pull-forward stories need the full flow-on-effects sweep.** Pulling Epic 9 stories 9.1.1 + 9.1.2 forward from v1.10–13 to v1.9.5.1 isn't just "build the code" — it touches release-schedule.ts (new key), Firestore feature docs (retarget + status: shipped via one-shot button), the dependency graph (downstream 9.x stories now reference a shipped dep — order checks remain healthy because `compareReleases` slots v1.9.5.1 between v1.9.5 and v1.10), the capacity packer (slots freed in v1.10–13 but no active re-pack needed — buffer absorbs), the release-state tables (CLAUDE.md + SESSION_HANDOVER.md), the in-app release-notes loader (auto-picks up the new .md file), and the user guide. When the user said "we also need to update all flow on effects as a result of this request" they were codifying the v1.6.2 lesson — cross-cutting changes need a checklist, not vibes.
...
- **One-shot retarget buttons can be self-contained components, not modules.** v1.9.5.1's `V1951RetargetButton` is a single ~80-line component dropped into `manage-organisation-page.tsx`'s new Fit-Up tab. No new admin module, no new permissions, no new menu entries — just a tile that lives at the top of an existing surface for one click. Pattern is leaner than the v1.10-restructure workbench (whole sheet + apply pipeline) for simple two-doc retargets. Keep one-shots proportional to the work.
