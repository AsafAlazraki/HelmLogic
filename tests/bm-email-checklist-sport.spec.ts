/**
 * "Mark's checklist" — Sport variant (SP600 with SP560 fallback).
 *
 * Sister spec to bm-email-checklist.spec.ts (which runs Classic CL380).
 * Same 8-item assertion matrix, different range so the release email can
 * include one PDF per major range showing the same auditable form.
 */
...
const OUT = 'test-results/bm-checklist-sport';
const MODULE_ID = 'M1Yf3R9igpJDxJnOVr6f';
const CARD = 'button.rounded-\\[1\\.5rem\\]';
const CUSTOMER_NAME = 'Mark Sport Checklist Customer';
const SPORT_MODELS = ['SP600', 'SP560']; // try the flagship, fall back to SP560
fs.mkdirSync(OUT, { recursive: true });

test.use({ viewport: { width: 1440, height: 900 } });

test("Mark's checklist — Sport end-to-end (SP600 / SP560)", async ({ page }) => {
    ...
    //    Try SP600 first; fall back to SP560 if the flagship isn't seeded. ──
    let onStep1 = false;
    let pickedModel = '';
    for (let attempt = 1; attempt <= 3 && !onStep1; attempt++) {
        ...
        await dlg.locator('.cursor-pointer:has-text("Sport")').first().click({ force: true });
        await page.waitForTimeout(1800);
        for (const m of SPORT_MODELS) {
            const modelCard = dlg.locator(`.cursor-pointer:has-text("${m}")`).first();
        ...
        if (!pickedModel) {
            console.log('▶ no SP600/SP560 in dialog — closing + retrying');
    ...
    expect(onStep1, `should land on Step 1 after Sport + ${pickedModel || 'SP600/SP560'}`).toBe(true);
    console.log('▶ picked Sport model:', pickedModel);
