...
export default function ModuleDetailsPage() {
    ...
    const recentQuotesQuery = useMemoFirebase(() =>
        user ? query(
            ...
            limit(20),  // v1.16 (gFQrcADO) — over-fetch so soft-deleted rows still leave 8 visible after filter
        ) : null,
    [firestore, user]);
    const { data: recentQuotesRaw } = useCollection<any>(recentQuotesQuery);
    /** v1.16 (gFQrcADO) — Hide soft-deleted quotes from the Recent Proposals
     *  grid. The actual delete sets `deletedAt`; the doc + sub-collections
     *  stay in Firestore so audit + sent emails are preserved. */
    ...
    // Trailers module — catalog/pricing/settings workspace
    if (moduleType === 'trailers' && moduleData && currentMemberOrg) {
        return (
            <div className="flex flex-col h-screen overflow-hidden bg-background">
                <div className="relative shrink-0 overflow-hidden bg-primary px-12 text-primary-foreground z-20 h-44 border-b-2 border-white/10">
                    <div className="absolute inset-0 z-0 bg-primary/95">
