...
function SortableModuleCard({ module, orgSlug }: { module: Module; orgSlug: string }) {
    const {
        attributes,
        listeners,
        setNodeRef,
...
function EmployeeDashboard({ organisationId, userProfile, orgSlug }: { organisationId: string; userProfile: UserProfile; orgSlug: string }) {
    ...
    if (orgLoading || modulesLoading) return <HelmLogicLoading label="Synchronizing Workspace" />;
                    ...
                    <div className="shrink-0 flex flex-col items-center md:items-end gap-2.5">
                        <div className="h-20 w-20 relative bg-white/90 backdrop-blur-sm rounded-3xl p-3 border border-white/40 shadow-2xl group hover:scale-105 transition-all">
                            {organisation?.primaryLogoUrl ? (
                                <div className="relative h-full w-full">
                                    <Image src={organisation.primaryLogoUrl} alt={`${organisation.name} logo`} fill className="object-contain p-2" />
                                </div>
                            ) : (
                        ...
                        <SortableContext items={orderedModules.map(m => m.id)} strategy={rectSortingStrategy}>
                            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {orderedModules.map((module) => (
                                    <SortableModuleCard key={module.id} module={module} orgSlug={orgSlug} />
                                ))}
                            </div>
                        </SortableContext>
                    </DndContext>
                    ...
                    <Card className="flex flex-col items-center justify-center h-80 border-2 border-dashed rounded-3xl bg-muted/5">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground text-center max-w-xs">
                            {userPermissions.can_access_module
                                ? "Your organisation hasn't subscribed to any modules yet."
...
export default function Dashboard() {
    ...
    if (isLoading || isAdmin) return <HelmLogicLoading label="Synchronizing Session" />;

    return (
        <div className="max-w-7xl mx-auto py-2">
            {organisationId && userProfile ? (
                <EmployeeDashboard organisationId={organisationId} userProfile={userProfile} orgSlug={orgSlug} />
            ) : (
