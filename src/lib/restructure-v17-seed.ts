...
const NEW_FEATURES: SeedFeature[] = [
    ...
    },
    {
        ...
        acceptanceCriteria: [
            'When quote.fitUp.included = true, PDF renders a "Fit-up" section after the items list and before totals',
            'Display options (admin setting per org): "Summary line only" OR "Itemised breakdown (item name + tier + cost per row)"',
            'Default = summary line',
            'Inc-GST shown alongside ex-GST per the global GST rule',
