...
const initialRanges = ['Open Boats', 'Prolines', 'Assualt Pros', 'Crossfire', 'Outlaws', 'Sea Masters', 'Wild Riders', 'Sea Runners', 'Plate'];
...
export function StacerDataStructure({ vendorId, vendorSlugOrId }: { vendorId: string, vendorSlugOrId: string }) {
    ...
    const handleSeedData = async () => {
        ...
        try {
            ...
            await batch.commit();
            toast({ title: 'Success', description: 'Initial Stacer data structure has been created.' });
        } catch (error) {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Stacer Data Structure</CardTitle>
            ...
            <CardContent>
                 {ranges && ranges.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed rounded-lg">
                        <p className="text-muted-foreground">No data structure found for Stacer.</p>
