        ...
        <section className="rounded-2xl border-2 bg-white p-4 space-y-3">
            ...
            {rows === null || rows === undefined ? (
                /* v1.34 pixel pass — skeleton rows hold the panel's shape
                   while loading instead of a lone micro-spinner. */
                <div className="space-y-2" aria-busy="true">
                    <div className="flex items-center gap-2 text-muted-foreground">
                        <Loader2 className="h-3.5 w-3.5 animate-spin" />
                        <span className="text-[10px]">Loading {tabMeta.label.toLowerCase()}…</span>
                    </div>
                    <div className="divide-y rounded-xl border-2 border-slate-200">
                        {[0, 1, 2, 3].map(i => (
                            <div key={i} className="flex items-center gap-3 px-3 py-2 animate-pulse">
                                <div className="h-9 w-12 rounded bg-slate-100 shrink-0" />
