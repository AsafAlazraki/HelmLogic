                    ...
                    <Form {...editForm}>
                        <form onSubmit={editForm.handleSubmit(handleUpdateRange)} className="space-y-4">
                            ...
                            <FormField control={editForm.control} name="image" render={({ field }) => (
                                <FormItem><Label>Range Image</Label><FormControl><Input type="file" accept="image/*" onChange={(e) => field.onChange(e.target.files?.[0])} /></FormControl><FormDescription>Upload a new image to replace the existing one.</FormDescription><FormMessage /></FormMessage>
                            )} />
                            <DialogFooter>
