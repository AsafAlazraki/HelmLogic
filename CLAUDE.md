...
| v1.10 (in progress) — Service Quoting + Fit-Up admin (full 9.1.x) + prod-bug pass | 🚧 Building on `claude/app-overview-wKiZ1`. Phase A — bug pass DONE (b516d4e): cover letter PDF restored (salespersonProfile fetch wired into `renderQuotePdf`), dealer-fit option names widened fallback + code backup, locked-quote discount honest-fail toast, stock-import duplicate race fixed (base36+random suffix). Phase B — Fit-Up admin DONE: new `organisations/{orgId}/fitUpItems/{itemId}` collection, new `Manage → Fit-Up Catalog` tab, full 9.1.x scope shipped (9.1.1 schema, 9.1.2 editor, 9.1.3 CSV import/export via xlsx, 9.1.4 bulk markup + retier + delete with confirm dialog). One-shot `V110RetargetButton` marks 9.1.1–9.1.4 as `shipped`/`v1.10`. Phase C — Service Quoting foundation (11.1.1 → 11.1.2 → 11.3.1) NEXT. **🚧 NOT yet in v1.10: quote-flow fit-up integration (Epic 9.2 = v1.16+), auto-classification (9.3.1 = v2.2), HL Save error + Import error (awaiting repros), RU200KAM $76.82 delta (→v1.11).** |
