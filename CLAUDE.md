...
| v1.11 — Fit-Up release (end-to-end quote-flow integration + catalog expansion + workshop status) | ✅ Ready for dev → main. **Phase A** — pulled Epic 9.2 forward from v1.16: `FitUpQuoteSelector` mounted on Step 5 of `highfield-quote-flow` with multi-level assignment (modules / brands / ranges / models AND-combined), motor-HP Suggested filter (simplified 9.3.1 — full rule engine still v2.2), customer PDF rolls up to a single "Fit-up & Rigging" summary line per locked Story 9.2.3 decision. **Phase B (expansion)** — `FitUpItem` schema gains `category` + `customerDescription` fields with admin filter chip row + free-text catalog category UI; new `organisations/{orgId}/fitUpPackages/{id}` collection bundles items into named one-click packages (Items / Packages sub-tabs in the catalog manager + Packages strip in the quote selector); quote selector gains free-text search + category chips + a per-line panel (qty stepper + per-quote price override + per-quote operator note); finalize snapshot + financials respect qty × (override ?? sellPrice ?? cost); new workshop `fitUpStatus` on the quote (`pending` / `scheduled` / `in-progress` / `complete`) renders as a popover pill on the proposal-view header beside the sales lifecycle pill — fully orthogonal — with audit-log entries via the new `fit-up-status-changed` event type. **NOT in v1.11 (pushed to v1.12)** — code IS on dev and will ship, but planning rows moved: Service Quote Flow (Epic 11.2 / 11.2.1), Service Catalog refinements (11.1.3 / 11.1.4), Motors Table read-view (3.7.3), Suggestion Approval Queue (3.5.1). The `V111ExpansionRetargetButton` did the bulk retarget; the new `fitUpPackages` rule needs a `firestore.rules` redeploy (rule body in `RELEASE_NOTES_v1.11.0.md`). `RELEASE_WINDOWS['v1.11'].shipped = true`. See `tasks/RELEASE_NOTES_v1.11.0.md` + `tasks/USER_GUIDE_v1.11.0.md`. |
