...
- **User Guide Format**: Every `USER_GUIDE_vX.Y.Z.md` file must follow the v1.7 template — title + audience line, at-a-glance map (table linking "what you want to do" → "where" → "section"), numbered sections per feature with "To do X" / "What this affects" / "Tips" sub-sections, a synthesis section ("How X works"), and a "What this release did NOT ship (deferred to vX.Y+1)" closer. Audience: org admins + salespeople. Tone: practical operator instructions, not engineering changelog. Same PR as the release. The v1.8 user guide is a known gap — backfilled OR explicitly skipped, but never repeat the oversight.
