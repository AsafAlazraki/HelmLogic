...
| v1.9 Quote-lifecycle wrap-up (Compatibility Rules + Inline Preview + State machine + Multiple Scenarios + SharePoint) | 🔄 In progress on `claude/app-overview-wKiZ1`. ~18 pts planned (under 20-pt cap). See `tasks/v1.9-build-plan.md`. **🚨 Email-dependent work explicitly OUT of v1.9** — 1.3.2 Contract Signing Pack + Viewed-via-tracking auto-transition both punted to v1.10 pending stakeholder email decisions. SharePoint write path will be env-flag-gated (`NEXT_PUBLIC_SHAREPOINT_ENABLED`) mirroring v1.8's email pattern. |
