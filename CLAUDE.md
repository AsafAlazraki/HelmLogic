...
| v1.9.5.1 Patch — prod-bug pass (4 fixes) + Epic 9 Fit-Up first slice (9.1.1 + 9.1.2 — schema + admin only) | ✅ Ready for dev → main. Bug pass: cover letter PDF restored (salespersonProfile fetch wired into `renderQuotePdf`), dealer-fit option names rendered with widened fallback + code backup, locked-quote discount save honest-fails with toast, stock-import duplicate race fixed with base36+random suffix. Fit-Up groundwork: new `organisations/{orgId}/fitUpItems/{itemId}` collection (schema: `name + tier + cost + sellPrice? + notes?`), new `Manage → Fit-Up Catalog` tab via `FitUpCatalogManager`, one-shot `V1951RetargetButton` that marks 9.1.1 + 9.1.2 as `shipped` in Firestore (cleanup commit removes it). Pulled 9.1.1 / 9.1.2 forward from v1.10–13; Service Quoting (Epic 11) still ships at v1.10 unchanged. `RELEASE_WINDOWS['v1.9.5.1'].shipped = true`. See `tasks/RELEASE_NOTES_v1.9.5.1.md` + `tasks/USER_GUIDE_v1.9.5.1.md`. **🚧 NOT shipped: quote-flow fit-up integration (Epic 9.2 = v1.16+), bulk CSV import (9.1.3 = Unscheduled), auto-classification (9.3.1 = v2.2), HL Save error + Import error (awaiting repros), RU200KAM $76.82 delta (→v1.11).** |
