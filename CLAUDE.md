...
| v1.10 (in progress) — Service Quoting catalogue + Fit-Up admin + prod-bug pass | 🚧 Building on `claude/app-overview-wKiZ1`. Phase A — bug pass DONE (b516d4e): cover letter PDF, dealer-fit names, locked-discount honest-fail, stock-import dupe race. Phase B — Fit-Up admin DONE (c6afa26): new `organisations/{orgId}/fitUpItems` collection, full Epic 9.1.x scope (9.1.1 schema, 9.1.2 editor, 9.1.3 CSV import/export, 9.1.4 bulk markup + retier + delete). Phase C — Service Quoting foundation DONE: two new collections `organisations/{orgId}/serviceOperations` (labor codes — `code + name + flatRateHours + hourlyRate + cost? + sellPrice? + notes?`, sell-price derives from hours × rate unless overridden) + `organisations/{orgId}/serviceParts` (parts catalog — `partNumber + name + cost + sellPrice? + stockLevel? + notes?`); new `Manage → Service Catalog` tab with sub-tabs (Operations / Parts), each with CRUD + CSV import/export (upsert by code / partNumber) + bulk markup + bulk delete. Stories 11.1.1 (schema) + 11.1.2 (catalogue admin) shipped. One-shot `V110RetargetButton` now marks all six stories (9.1.1–9.1.4 + 11.1.1 + 11.1.2) as `shipped`/`v1.10`. Gating fix (`ef16825`): controlled Tabs + lazy-mount the Fit-Up + Service Catalog content so a missing rule on either collection can't white-screen `/manage`. **🚧 NOT yet in v1.10: Service-quote create form / dashboard / PDF / lifecycle (Epic 11.2 = v1.11+), 11.3.1 customer reconciliation + NSM-Hub migration (deferred — needs `nsm-service-quotation` service-account), module x3 + parts x1 stories from original v1.10 plan (retargeting to v1.11 — specific story IDs to be selected at v1.11 kickoff), quote-flow fit-up integration (Epic 9.2 = v1.16+), auto-classification (9.3.1 = v2.2), HL Save error + Import error (awaiting repros), RU200KAM $76.82 delta (→v1.11).** |
